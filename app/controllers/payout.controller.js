...
module.exports = {
    createPayout: async (req, res) => {
        ...
        if (!wallet)
            return res
                .status(404)
                .json({ status: false, message: "Wallet address not found" });
        if (userId !== wallet.user._id)
            return res
                .status(403)
                .json({
                    status: false,
                    message:
                        "User making request does not match wallet user ID",
                });
    ...
    getPayoutHistory: async (req, res) => {
        ...
        try {
            ...
            return res.status(500).json({
                status: false,
                message:
                    error.message || "Error occured while retrieving payouts.",
            });
        }
    },
};
